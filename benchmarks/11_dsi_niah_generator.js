...
// The Needle
const NEEDLE_SECRET = "DELTA-TANGO-77";
const NEEDLE_QUESTION = "What is the override password for sector 7G?";
...
for (let i = 0; i < TOTAL_FILES; i++) {
  ...
  for (let j = 0; j < LINES_PER_FILE; j++) {
    if (i === NEEDLE_FILE_INDEX && j === NEEDLE_LINE_INDEX) {
      fileLines.push(`[CRITICAL] The override password for sector 7G is ${NEEDLE_SECRET}. Do not share this under any circumstances.`);
    } else {
  ...
  const reason = (i === NEEDLE_FILE_INDEX) ? "system+security+passwords+overrides" : "system+network+timeouts+retries";
...
console.log("\n📍 NEEDLE LOCATION (SPOILER):");
console.log(`File: niah_sys_log_${String(NEEDLE_FILE_INDEX).padStart(4, '0')}.md`);
console.log(`Line: ${NEEDLE_LINE_INDEX}\n`);
console.log("Ready for Claude Code to execute the retrieval challenge!");
